...
from models.xray_resnet import XrayResNet  # Updated ResNet model with uncertainty-based weighting
from trainers.mtl_kendal_train import MTLTrainerWithUncertainty  # Updated trainer for uncertainty-based weighting
...
parser = argparse.ArgumentParser(description="Train a multi-task learning (MTL) model with Kendall's uncertainty weighting")
...
model_registry = {
    "xray": {
        "ResNet": lambda img_size, df: XrayResNet(img_size, df)  # ResNet with uncertainty weighting
    }
}
...
# Initialize and train the model using the uncertainty-based trainer
trainer = MTLTrainerWithUncertainty(model, train_loader, val_loader, optimizer, project_name="MTL_Kendal_Training")
trainer.train(num_epochs=num_epochs, model_path=save_model_path)

# Test the model
tester = MTLTester(model, test_loader, nn.CrossEntropyLoss(), "MTL_Uncertainty_Test")
tester.test()
