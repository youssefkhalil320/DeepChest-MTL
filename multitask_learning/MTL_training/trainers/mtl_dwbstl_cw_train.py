...
class MTLTrainerWithDWBSTLCW(MTLTrainer):
    def __init__(self, model, train_loader, val_loader, optimizer, criterion, initial_weights, project_name="MTL_Training"):
        ...
        self.task_class_weights = self.calculate_class_weights(train_loader)
        self.criterion = nn.CrossEntropyLoss  # Keep the criterion flexible to include weights

    # def calculate_class_weights(self, loader):
    #     """
    #     Calculate class weights for each task based on the training loader.
    #     """
    #     task_class_weights = {}
    #     for _, labels in loader:
    #         for i, task in enumerate(self.model.task_names):
    #             class_counts = Counter(labels[:, i].cpu().numpy())
    #             total_samples = sum(class_counts.values())
    #             weights = {cls: total_samples / count for cls, count in class_counts.items()}
    #             weight_tensor = torch.tensor([weights[cls] for cls in weights.keys()], dtype=torch.float)
    #             task_class_weights[task] = weight_tensor
    #     return task_class_weights

    # def calculate_class_weights(self, loader):
    #     """
    #     Optimized function to calculate class weights for each task based on the entire dataset.
    #     """

    #     task_class_counts = defaultdict(Counter)

    #     # Aggregate labels across all batches
    #     for _, labels in loader:
    #         for i, task in enumerate(self.model.task_names):
    #             task_class_counts[task].update(labels[:, i].cpu().numpy())

    #     # Calculate weights for each task
    #     task_class_weights = {}
    #     for task, class_counts in task_class_counts.items():
    #         total_samples = sum(class_counts.values())
    #         weights = {cls: total_samples / count for cls, count in class_counts.items()}
    #         weight_tensor = torch.tensor([weights[cls] for cls in sorted(weights.keys())], dtype=torch.float)
    #         task_class_weights[task] = weight_tensor

    #     return task_class_weights

    def calculate_class_weights(self, loader, log_file="class_weights.log"):
        """
        Optimized function to calculate class weights for each task 
        based on the entire dataset in the loader, with a progress bar.
        Logs the calculated class weights into a file.
        """
        ...
        logging.info("Starting calculation of class weights.")

        # Aggregate class counts for all tasks
        task_class_counts = defaultdict(Counter)

        # Iterate over the entire dataset and aggregate labels with a progress bar
        for _, labels in tqdm(loader, desc="Calculating class weights"):
            labels = labels.cpu().numpy()  # Move labels to CPU for processing
            for i, task in enumerate(self.model.task_names):
                task_class_counts[task].update(labels[:, i])

        # Calculate class weights for each task
        task_class_weights = {}
        for task, class_counts in task_class_counts.items():
            total_samples = sum(class_counts.values())
            max_classes = max(class_counts.keys()) + 1  # Dynamically determine the number of classes
            weights = {
                cls: total_samples / count if count > 0 else 0 
                for cls, count in class_counts.items()
            }
            # Create a tensor for class weights
            weight_tensor = torch.zeros(max_classes, dtype=torch.float)
            for cls, weight in weights.items():
                weight_tensor[cls] = weight
            task_class_weights[task] = weight_tensor

            # Log the weights for the task
            logging.info(f"Task: {task}, Class Weights: {weights}")

        logging.info("Finished calculating class weights.")
    ...
    def train(self, num_epochs=10, model_path="fashion_classifier.pth"):
        wandb.init(project=self.project_name, config={
            ...
            "learning_rate": self.optimizer.param_groups[0]['lr']  # Ensure this comes from the optimizer
        })
        ...
        for epoch in range(num_epochs):
            ...
            for imgs, labels in tqdm(self.train_loader, desc=f"Epoch {epoch + 1}/{num_epochs} - Training"):
                ...
                for i, (out, task) in enumerate(zip(outputs, self.model.task_names)):
                    # Use task-specific class weights
                    class_weights = self.task_class_weights[task].to(out.device)
                    task_loss_fn = nn.CrossEntropyLoss(weight=class_weights)
                    
                    # Compute weighted loss for the task
                    task_loss = self.dynamic_weights.loss_weights[task] * task_loss_fn(out, labels[:, i])
            ...
            with torch.no_grad():
                for imgs, labels in tqdm(self.val_loader, desc=f"Epoch {epoch + 1}/{num_epochs} - Validation"):
                    ...
                    for i, (out, task) in enumerate(zip(outputs, self.model.task_names)):
                        ...
                        # Compute loss
                        task_loss = task_loss_fn(out, labels[:, i])
