...
class BotitResNet(nn.Module):
    def __init__(self, img_size, df):
        super(BotitResNet, self).__init__()
        ...
        # This leaves the model with all layers up to (but not including) the classifier
        self.resnet50_features = nn.Sequential(*list(self.resnet50.children())[:-1])

        # Define the flattened size after the feature extraction
        # ResNet50 outputs a 2048-dimensional vector for each image
        self.flat_size = 2048
    ...
    def forward(self, x):
        # Pass input through ResNet50's feature extractor
