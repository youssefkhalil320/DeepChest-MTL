...
class XraySwinTransformer(nn.Module):
    def __init__(self, img_size, df, swin_version='swin_base_patch4_window7_224'):
        super(XraySwinTransformer, self).__init__()
        self.img_size = img_size
        self.df = df

        # Load the Swin Transformer model from timm
        self.backbone = create_model(swin_version, pretrained=True)
