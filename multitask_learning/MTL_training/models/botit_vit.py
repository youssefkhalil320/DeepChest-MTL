...
class BotitViT(nn.Module):
    def __init__(self, img_size, df):
        super(BotitViT, self).__init__()
        ...
        # Extract the hidden dimension from the ViT's head
        hidden_dim = self.vit.heads.head.in_features

        # Replace the head to output feature embeddings
    ...
    def forward(self, x):
        ...
        x = self.vit(x)  # Outputs feature embeddings
