...
class XrayViT(nn.Module):
    def __init__(self, img_size, df, vit_version='vit_b_16'):
        super(XrayViT, self).__init__()
        ...
        if vit_version == 'vit_b_16':
            ...
            raise ValueError(f"Unsupported ViT version: {vit_version}")

        # Extract the number of features from the head
        num_features = self.backbone.heads.head.in_features

        # Replace the head with an identity layer to extract features
