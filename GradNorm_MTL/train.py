...
from models.xray_resnet import XrayResNet  # Updated ResNet model for GradNorm
from trainers.mtl_grad_norm_trainer import MTLTrainerWithGradNorm  # GradNorm-based trainer
...
parser = argparse.ArgumentParser(description="Train a multi-task learning (MTL) model with GradNorm")
...
parser.add_argument('--dataset_name', type=str, default="xray", choices=["xray"], help="Dataset name")
...
parser.add_argument('--alpha', type=float, default=0.12, help="GradNorm alpha value")
...
model_registry = {
    "xray": {
        "ResNet": lambda img_size, df: XrayResNet(img_size, df)  # ResNet with GradNorm support
    }
}
...
# Initialize and train the model using the GradNorm-based trainer
trainer = MTLTrainerWithGradNorm(model, train_loader, val_loader, optimizer, nn.CrossEntropyLoss(), alpha=alpha, project_name="MTL_GradNorm_Training")
trainer.train(num_epochs=num_epochs, model_path=save_model_path)

# Test the model
tester = MTLTester(model, test_loader, nn.CrossEntropyLoss(), "MTL_GradNorm_Test")
tester.test()
